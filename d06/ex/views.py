...
def like(request, tip_id):
    ...
    # like
        ...
        if tip.likes.count() > 1:
            print("!grant users right to upvote")
            permission = Permission.objects.get(codename='can_be_downvote')
            tip.author.user_permissions.add(permission)

    return redirect("ex:index")

@login_required(login_url='/login/') # чет не работает :/
def dislike(request, tip_id):
    tip = get_object_or_404(TipDay, pk=tip_id)

    # dislike
    if request.user == tip.author or request.user.has_perm("ex.can_be_downvote"): 
